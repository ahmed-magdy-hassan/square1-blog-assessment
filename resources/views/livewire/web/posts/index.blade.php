        ...
        <div class="ml-3 relative flex items-center space-x-2">
            <x-jet-dropdown align="left" width="48">
                ...
                <x-slot name="content">
                    ...
                    <x-jet-dropdown-link wire:click.prevent="sortBy('publication_date','asc')">
                        {{ __('Oldest') }}
                    </x-jet-dropdown-link>

                    <x-jet-dropdown-link wire:click.prevent="sortBy('publication_date','desc')">
                        {{ __('Newest') }}
                    </x-jet-dropdown-link>
                </x-slot>
            </x-jet-dropdown>
            <span class="inline-flex rounded-md">
                <button wire:click.prevent="sortBy('publication_date')" type="button" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition">
                    Rest
            ...
            <div class="py-12">
                <article class="space-y-2 gap-4 xl:grid xl:grid-cols-4 xl:space-y-0">
                    ...
                    <div class="space-y-5 xl:col-span-3">
                        <div class="space-y-6">
                            <h2 class="flex items-center space-x-2 text-2xl leading-8 font-bold tracking-tight">
                                <a class="text-gray-900 hover:text-blue-700 font-sans" href="{{route('posts.show',$post->id)}}">{{$post->title}}</a>
                                @if ($post->publication_date > now())
                                    <span class="block flex-none uppercase bg-red-100 text-red-600 text-xs tracking-wide font-semibold px-2 py-1 rounded-md ml-3">
                                        Not published yet 
                                    </span>
                                @endif
                            </h2>
